namespace Terrasoft.Configuration.ICLEntityEvents
{
	...
	public class CaseEntityEventListener : BaseEntityEventListener
	{
		...
		/// <summary>
		/// Автозаполнение "Фактическое время разрешения (часы)" (ICLActualResolutionTime) 
		/// </summary>
		/// <param name="entity">
		private void SetICLActualResolutionTime(Entity entity)
		{
			var caseCategoryId = entity.GetTypedColumnValue<Guid>("CategoryId");
			var changeRequest = ServerConstants.CaseCategory.ChangeRequest;
			var changed = entity.GetChangedColumnValues();
			if (changed.Any(col => col.Name == "Category"))
			{
				//Запрос на изменение
				if(caseCategoryId != Guid.Empty && caseCategoryId == changeRequest)
				{
					//Фактическая реакция
					DateTime respondedOn = entity.GetTypedColumnValue<DateTime>("RespondedOn");
					//Фактическое разрешение
					DateTime solutionProvidedOn = entity.GetTypedColumnValue<DateTime>("SolutionProvidedOn");
					if(respondedOn != DateTime.MinValue && solutionProvidedOn != DateTime.MinValue)
					{
						TimeSpan diffTime = respondedOn - solutionProvidedOn;
						double diffHours = Math.Abs(diffTime.TotalHours);
						entity.SetColumnValue("ICLActualResolutionTime", Math.Round(diffHours, 2));
					}
				}
			}
